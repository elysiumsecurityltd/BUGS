...
printf("\n BUGS ALGORITHM LIGHT UNCRYPT FILE ");
	   ...
	   do
		...
		i++;
		 ...
		 crypt_time = (int)(clock()/CLOCKS_PER_SEC);
                 ...
                 if(bfile(1,tempname,PARAM_TARGET, "",pass_clear, length,
	           ...
	           printf(" The crypted file has been renamed : %s \n",tempname);
...
int argcheck(int argc, char **argv)
	...
	if (argc > 1)
	{
	 ...
	 if ((strcmp(argv[i],"-pwd") == 0) && (i+1 < argc))
		...
		{
		PARAM_ERROR = 1;
		strcpy(PARAM_ERRORFILE,"bunlock.log");
                PARAM_ERRORFILE[strlen("bunlock.log")]='\0';
		}
	 else 
             {
...
void usage()
     {   
      printf("\n BUNLOCK %s, (C) Martinez Sylvain",VERSION);
      printf("\n BUGS ALGORITHM LIGHT UNCRYPT FILE");
      printf("\n\n Usage: bunlock SOURCE_FILE {OPTIONS}");
